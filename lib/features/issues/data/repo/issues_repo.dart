...
class IssuesRepo {
  ...
  // Stream issue details
  Stream<Either<Failure, IssueResponseModel?>> streamIssueDetails(
      String issueId) async* {
    try {
      final local = await _local.get(issueId);
      if (local != null) {
        yield Right(local);
      }

      yield* Rx.merge([
        if (isConnected) _handleIssueDetailsStream(true, local, issueId),
        _netManager.connectionStream
            .distinct()
            .where((connected) => connected)
            .asyncExpand((_) => _handleIssueDetailsStream(true, local, issueId))
      ]);
    } on Failure catch (e) {
      yield Left(e);
    } catch (e) {
      yield Left(CustomFailure("تعذر تحميل بيانات العطل."));
    }
  }

  Stream<Either<Failure, IssueResponseModel?>> _handleIssueDetailsStream(
      bool isConnected, IssueResponseModel? local, String issueId) async* {
    if (!isConnected) return;

    final remoteStream =
        _remote.fetchIssueDetails(issueId).handleError((error) {
      if (error is RealtimeSubscribeException) {
        Future.delayed(Duration(seconds: 5), () {
          _remote.fetchIssueDetails(issueId);
        });
      }
    });
